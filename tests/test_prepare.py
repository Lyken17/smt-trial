...
class PrepareTests(unittest.TestCase):
    def test_official_single_query_limits_in_xml(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            output = root / "run.xml"
            prepare("SingleQuery", ROOT / "configs/cvc5/single-query.toml", cvc5, selection.parent, output)
    ...
    def test_division_filters_task_logics(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            prepare(
                "SingleQuery",
                ROOT / "configs/cvc5/single-query.toml",
                cvc5,
                root / "selection",
                output,
    ...
    def test_incremental_uses_official_parser(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            prepare(
                "Incremental",
                ROOT / "configs/cvc5/incremental.toml",
                cvc5,
                selection.parent,
                output,
