...
EXTERNAL_CACHE_ROOT=/data/smtcomp-cache SELECTION_JOBS=8 \
  make setup-single-query
```
该精简入口不会安装 Docker、Dolmen、trace executor，也不会下载 incremental 数据；
需要全部 Track 时必须使用 `make setup-all`。
...
固定官方归档在 `QF_LinearIntArith` 的跨 Track 回归结果：

| Track | performance | solver | error | correctly solved score |
|---|---|---|---:|---:|
| SingleQuery | `24` | cvc5 | 0 | 3,782 |
| Incremental | `par` | cvc5 | 0 | 3,879,902 |
| UnsatCore | `par` / `seq` | cvc5 | 0 | 2,189,130 |
| ModelValidation | `par` / `seq` | cvc5 | 0 | 3,815 |
| Parallel | `par` | SMTS | 0 | 28 |

Parallel 行使用 SMTS，因为官方 cvc5 submission 没有参加 2025 Parallel；本仓库的
cvc5 Parallel 是按官方数据/限制进行的新调参实验，不能伪称当年官方参赛成绩。
结果来源分别为官方 `results-{sq,inc,uc,mv,parallel}-2025.json.gz`：
...
全部 Track 的组织者归档也可直接回归，例如：

```bash
make score TRACK=Incremental DIVISION=QF_LinearIntArith PERFORMANCE=par \
  RESULTS=.cache/official/data/results-inc-2025.json.gz
make score TRACK=UnsatCore DIVISION=QF_LinearIntArith PERFORMANCE=seq \
  RESULTS=.cache/official/data/results-uc-2025.json.gz
make score TRACK=ModelValidation DIVISION=QF_LinearIntArith PERFORMANCE=par \
  RESULTS=.cache/official/data/results-mv-2025.json.gz
make score TRACK=Parallel DIVISION=QF_LinearIntArith PERFORMANCE=par \
  RESULTS=.cache/official/data/results-parallel-2025.json.gz
```
